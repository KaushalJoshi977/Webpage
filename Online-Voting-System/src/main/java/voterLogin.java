...
//           System.out.println(voternum);
           ...
           {
        	PreparedStatement preparedStatement2 = con.prepareStatement("select vn from nvoter where vn = '"+voternum+"'");
            ResultSet rs =   preparedStatement2.executeQuery("select vn from nvoter where vn = '"+voternum+"'");
            if(rs.next()){
                response.sendRedirect("userWelcome.jsp?con=");
            }else{
